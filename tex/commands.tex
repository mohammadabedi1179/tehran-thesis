...
\usepackage{etoolbox}
% بستهٔ سازگار با نسخه‌های جدید LaTeX برای بازنشانی شمارهٔ پاورقی در ابتدای هر صفحه
\usepackage{zref-perpage}
% برای حذف وابستگی شمارندهٔ پاورقی به فصل
\usepackage{chngcntr}
...
% گزینه‌های hyperref باید پیش از بارگذاری بسته تعیین شوند.
% بسته hyperref فقط یک‌بار بارگذاری می‌شود تا خطای Option clash رخ ندهد.
% گزینه hidelinks همهٔ لینک‌ها را فعال نگه می‌دارد، اما متن آن‌ها را مشکی
% و کادر اطراف ارجاعات، پاورقی‌ها، لینک‌های وب و عبارات فهرست را حذف می‌کند.
\ifdraft
{%
	\PassOptionsToPackage{unicode=true,pagebackref=true,hidelinks,final}{hyperref}%
}
{%
	\PassOptionsToPackage{unicode=true,pagebackref=false,hidelinks}{hyperref}%
}
\usepackage{hyperref}
\usepackage{qrcode}
...
% در کلاس book، شمارندهٔ پاورقی به‌طور پیش‌فرض با شروع هر فصل صفر می‌شود.
% ابتدا این وابستگی حذف می‌شود و سپس شمارهٔ پاورقی در ابتدای هر صفحه از ۱ آغاز می‌شود.
\counterwithout{footnote}{chapter}
\zmakeperpage{footnote}
% برای تثبیت شماره‌ها پس از تغییر صفحه‌بندی، سند باید دست‌کم دو بار کامپایل شود.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
% \SepMark{-}
\DeclareRobustCommand{\serialcmd}{\lr{\texttt{SP\char35{}M0\char35{}M1\char35{}M2\char35{}M3}}}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% شماره‌گذاری صفحات پیوست‌ها
%% صفحات متن اصلی بدون تغییر و به‌صورت پیوسته شماره‌گذاری می‌شوند.
%% از آغاز پیوست‌ها، شمارهٔ نمایشی هر صفحه به صورت «آ.۱، آ.۲، ...»
%% و برای پیوست بعدی به صورت «ب.۱، ب.۲، ...» خواهد بود.
%% شمارندهٔ واقعی page بازنشانی نمی‌شود؛ بنابراین شمارش کل صفحات و
%% مقصدهای پیوندهای PDF دچار تکرار یا اختلال نمی‌شوند.

\newif\ifappendixpagenumbering
\appendixpagenumberingfalse
\newcounter{appendixstartpage}

% شمارهٔ محلی صفحه در پیوست جاری:
% صفحهٔ واقعی فعلی - صفحهٔ آغاز پیوست + ۱
\newcommand{\appendixlocalpagenumber}{%
	\number\numexpr\value{page}-\value{appendixstartpage}+1\relax
}

% در این بخش، حالت makeatletter از تنظیمات الگوریتم فعال است.
% با اجرای \appendix، حالت شماره‌گذاری ویژهٔ پیوست فعال می‌شود.
\pretocmd{\appendix}{\appendixpagenumberingtrue}{}{%
	\PackageWarning{tehran-thesis}{Unable to patch \string\appendix}%
}

% این دستور فقط برای فصل‌های شماره‌دار اجرا می‌شود؛ بنابراین در آغاز هر
% پیوست، مبدأ شمارهٔ محلی همان صفحه ثبت می‌شود.
\pretocmd{\@makechapterhead}{%
	\ifappendixpagenumbering
		\setcounter{appendixstartpage}{\value{page}}%
		\gdef\thepage{\thechapter.\appendixlocalpagenumber}%
	\fi
}{}{%
	\PackageWarning{tehran-thesis}{Unable to patch \string\@makechapterhead}%
}
...
% صفحات زوج: عنوان فصل + عنوان پایان‌نامه
% صفحات فرد: فقط عنوان پایان‌نامه

\fancyhead{}

% صفحه زوج: نام/عنوان فصل در سمت راست، عنوان پایان‌نامه در سمت چپ
\fancyhead[ER]{\small\nouppercase{\leftmark}}

% صفحه فرد: فقط عنوان پایان‌نامه
\fancyhead[OR]{\footnotesize\@title}
